...
public class CollapsingFloor1 : MonoBehaviour
{
    ...
    {
        // After .5 seconds a warning sound will play indicating the floor is soon to fall.
        yield return new WaitForSeconds(.5f);
        warningSound.Play();

        // After 1 second, then isKinematic is set to false, collider is set to false, and sound is played.
        yield return new WaitForSeconds(1f);
        rb.isKinematic = false;
        GetComponent<Collider>().enabled = false;
        CollapseSound.Play();

        // After 3 seconds, then isKinematic is set to true, collider is set to true, game object is set back to starting position, and sound is played.
